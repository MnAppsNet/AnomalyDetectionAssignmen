...
if __name__ == '__main__':
    ...
    # periods_dfs contain one df for each trace.
    # labels contain information for the anomaly Instances (109) incosistent with paper, essentially labels describe a range of anomalies (section of trace) 
    # peridos info contain info for each trace has the same len with periods_dfs. [file_name,trace_type] trace type one of 6 anomalies or normal. 
    ...
    # get the information from labels and anomaly ranges in traces and add a column with the type of anomaly
    ...
    # essentially ensures that the data will be one per second.
    ...
    # this remove the first and last instances from periods
