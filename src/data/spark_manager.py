...
class SparkManager(DataManager):
    ...
    def load_raw_data(self, data_paths_dict):
        ...
        # change again to previus
        app_keys = [f'app{self.app_id}'] if self.app_id != 0 else [f'app{i}' for i in set(APP_IDS) - {2,3,4,5,6,7, 8,9,10}]
        ...
        for app_key in app_keys:
            ...
            for trace_type in self.trace_types:
                ...
                if len(type_file_names) > 0:
                    ...
                    print('done.')
            #print(period_dfs[0].shape)
            #print(period_dfs[0].head())
            #print(len(periods_info))
            #print(periods_info)
            #print(len(period_dfs))
